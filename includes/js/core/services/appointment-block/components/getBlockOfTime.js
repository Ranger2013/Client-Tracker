 ...
 */
export default async function getBlockOfTime({avgHorses, numberHorses, avgDriveTime, time = null}) {
	try {
		let horsesPerHour = 60 / avgHorses; // can do 1 horse in x mins
		let driveTime = 60 * avgDriveTime; // Converts to seconds
		let workTime = (horsesPerHour * 60) * numberHorses; // Converts to seconds should be times the number of horses the user has.
		...
		AppError.process(err, {
			errorCode: AppError.Types.CALCULATION_ERROR,
			userMessage: 'Unable to calculate the end time of the appointment block.',
		}, true);
	}
} 