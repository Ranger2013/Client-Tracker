 ...
 */
export default async function getProjectedAppointments(trimDate, trimCycle, clientInfo, scheduleOptions) {
	try {
		...
		// Set up the Dates
		const [year, month, day] = trimDate.value.split('-');
			...
			if(trimDates && trimDates.length > 0){
				for(const trimDate of trimDates){
					...
					const bookingData = {
						...
						time_block: await getBlockOfTime(scheduleOptions.avg_horses, numHorses, scheduleOptions.avg_drive_time),
					};
		...
		await errorLogs('getProjectedAppointmentsError', 'Get projected appointments error: ', err);
		throw err;
	}
}