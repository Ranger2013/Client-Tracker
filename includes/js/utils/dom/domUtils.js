...
/**
 * Renders elements to a container in a specified order
 * @param {HTMLElement} mainContainer - The main container to render into
 * @param {Object} elements - Object containing elements to render
 * @param {Object} [options] - Optional object specifying render options
 * @param {string[]} [options.renderOrder] - Optional array specifying render order of element keys
 * @param {HTMLElement} [options.container] - Optional container to render elements into
 * @param {boolean} [options.clearContainer=true] - Whether to clear the main container before rendering
 * @param {Function} [options.beforeRender] - Function to call before rendering elements
 * @param {Function} [options.afterRender] - Function to call after rendering elements
 * @param {Function} [options.errorHandler] - Function to call if an error occurs during rendering
 */
export function renderPage(mainContainer, elements, options = {}) {
    const {
        renderOrder = [],
    ...
    try {
        if (clearContainer) mainContainer.innerHTML = '';
        if (beforeRender) beforeRender(container);

        const order = renderOrder.length ? renderOrder : Object.keys(elements);
        order.forEach(key => {
        ...
        if (container !== mainContainer) mainContainer.appendChild(container);
    } catch (err) {
